  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between h-16">
        ...
        <button className="md:hidden" onClick={() => setMobileOpen(!mobileOpen)} style={{ color: '#3B2317', background: 'none', border: 'none' }}>
        ...
        <div className="md:hidden px-6 pb-6 flex flex-col gap-4" style={{ backgroundColor: 'rgba(250, 248, 244, 0.98)' }}>
          {NAV_ITEMS.map((item) =>
            item.href ? (
              ...
              <button key={item.label} onClick={() => handleNavClick(item.target!)} className="font-sans text-sm uppercase tracking-[0.15em] py-2 text-left" style={{ color: '#3B2317', background: 'none', border: 'none' }}>
                {item.label}
              </button>
            )
          )}
