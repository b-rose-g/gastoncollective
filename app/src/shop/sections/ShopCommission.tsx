...
export default function ShopCommission() {
  ...
  const handleFileDrop = (e: React.DragEvent) => {
    ...
    const newFiles = dropped.map((f) => URL.createObjectURL(f));
    setFiles((prev) => [...prev, ...newFiles].slice(0, 3));
  };

  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selected = Array.from(e.target.files || []).filter((f) => f.type.startsWith('image/'));
    const newFiles = selected.map((f) => URL.createObjectURL(f));
    setFiles((prev) => [...prev, ...newFiles].slice(0, 3));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createCommission.mutate({
      ...
      referenceImages: files.length > 0 ? files.join(',') : undefined,
    });
  };
    ...
    <section id="commission" ref={sectionRef} className="relative" style={{ backgroundColor: '#F8F0FF' }}>
      <div className="relative z-10 max-w-[800px] mx-auto px-6 md:px-12 py-32 md:py-40">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="opacity-0">
          ...
          <div className="mb-6">
            <label className="font-sans text-xs uppercase tracking-[0.15em] block mb-2" style={{ color: '#5A4A6E', opacity: 0.6 }}>What are you looking for? *</label>
            <select name="commissionType" value={formData.commissionType} onChange={handleChange} required
          ...
          <div className="mb-6">
            <label className="font-sans text-xs uppercase tracking-[0.15em] block mb-2" style={{ color: '#5A4A6E', opacity: 0.6 }}>Tell me about your vision *</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              <label className="font-sans text-xs uppercase tracking-[0.15em] block mb-2" style={{ color: '#5A4A6E', opacity: 0.6 }}>Size / Format</label>
            ...
            <div>
              <label className="font-sans text-xs uppercase tracking-[0.15em] block mb-2" style={{ color: '#5A4A6E', opacity: 0.6 }}>Budget</label>
          ...
          {/* File Upload */}
          <div className="mb-10">
            <label className="font-sans text-xs uppercase tracking-[0.15em] block mb-3" style={{ color: '#5A4A6E', opacity: 0.6 }}>Reference Images (optional, max 3)</label>
            <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleFileDrop}
              ...
              <input id="commission-file-input" type="file" accept="image/*" multiple onChange={handleFileSelect} className="hidden" />
            </div>
            {files.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-4">
                {files.map((file, i) => (
                  <div key={i} className="relative group" style={{ width: 80, height: 80 }}>
                    <img src={file} alt="" className="w-full h-full object-cover" style={{ border: '1px solid #E8DDD0', borderRadius: 8 }} />
          ...
          <div className="flex flex-col items-center gap-4">
            <button type="submit" disabled={createCommission.isPending}
              ...
              {createCommission.isPending ? 'Sending...' : submitted ? 'Request Sent!' : 'Submit Request'}
            </button>
            {submitted && (
